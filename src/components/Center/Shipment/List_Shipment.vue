            ...
            <table class="table">
              ...
              <tbody>
                <tr v-for="items in item" :key="items.id">
                  ...
                  <td><span class="text-success" v-for="services in items.Services" :key="services">{{services}}-</span></td>
                  <td><span class="text-success">{{items.Evaloation}}</span></td>
                  <td><span class="text-success" v-for="type in items.Type_Of" :key="type">{{type}}-</span></td>
                  <td><span class="text-success">--</span></td>
                  <td><span class="text-success">{{items.Weight}}</span></td>
                  <td><span class="text-success">{{items.DistinationCity}}-{{items.DistinationLocation }}</span></td>
                </tr>
              </tbody>
            </table>
          </div>
...
export default {
  ...
  methods: {
    async Create() {
      await axios.get('http://localhost:3000/mangecenter/find_shipment_center',{withCredentials: true}).then((res) => {
        this.item = res.data
